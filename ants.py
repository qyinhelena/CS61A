    ...
    def nearest_bee(self, beehive):
        ...
        def helper(place, hive, count):
            while place is not hive:
                ...
                return helper(place.entrance, hive, count+1)
            return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.place.bees: # if there is still bee alive
            for bee in bees_copy:
                bee.reduce_armor(amount)
        self.armor -= amount
        if self.armor <= 0: # if fireant dies, bonus damage to all bees
            self.place.bee.reduce_armor(self.damage) # damage the bee witht the damage attribute of fireant before fireant is removed
            self.place.remove_insect(self) # remove fireant from the place
        else: # if fireant doesn't die, reflected damage to all bees
            self.place.bee.reduce_armor(amount)
