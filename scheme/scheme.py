...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        outcome = scheme_eval(first, env)
        check_procedure(outcome)
        if not isinstance(outcome, MacroProcedure):
            operand = rest.map(lambda x: scheme_eval(x, env))
            return scheme_apply(outcome, operand, env)
        return scheme_eval(outcome.apply_macro(rest, env), env)
        # END PROBLEM 4
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        elif self.parent: # lookup parent frame if parent is not None
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('the number of vals are not matched with number of formals')
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('Error with arguments. Please change.')
        # END PROBLEM 3
...
def make_let_frame(bindings, env):
    ...
    vals = bindings.map(lambda y: y.first)
    check_formals(formals)
